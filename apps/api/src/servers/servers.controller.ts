...
export class ServersController {
  ...
  async create(@Body() dto: CreateServerDto) {
    if (!dto.nodeId || !dto.templateSlug || !dto.name) {
      throw new BadRequestException('nodeId, templateSlug, and name are required');
    }
    ...
    try {
      ...
      await this.commands.send(dto.nodeId, { type: 'command.startContainer', serverId: server.id });
    } catch (err) {
    ...
    void this.commands
      .send(dto.nodeId, { type: 'command.streamLogs', serverId: server.id, follow: true })
      .catch((err) => this.logger.warn(`Failed to start log streaming for ${server.id}: ${err.message}`));
  ...
  async start(@Param('id') id: string) {
    ...
    await this.commands.send(server.nodeId, { type: 'command.startContainer', serverId: id });
  ...
  async stop(@Param('id') id: string) {
    const server = this.servers.getOrThrow(id);
    await this.commands.send(server.nodeId, { type: 'command.stopContainer', serverId: id });
    return this.servers.get(id);
  }
  ...
  async remove(@Param('id') id: string) {
    const server = this.servers.getOrThrow(id);
    await this.commands.send(server.nodeId, { type: 'command.deleteContainer', serverId: id });
  ...
  private resolveEnv(
    envSchema: { key: string; default?: string | number | boolean; required?: boolean }[],
    overrides: Record<string, string>,
  ): Record<string, string> {
    const env: Record<string, string> = {};
    for (const field of envSchema) {
      const value = overrides[field.key] ?? (field.default !== undefined ? String(field.default) : undefined);
      if (value === undefined) {
        if (field.required) throw new BadRequestException(`Missing required field "${field.key}"`);
        continue;
      }
      env[field.key] = value;
    }
